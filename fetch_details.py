...
def main():
    ...
    while True:
        # 페이지 번호(?page=N)를 순차적으로 증가시키며 요청
        ...
        # 해당 페이지에 데이터가 없으면 수집 종료
        if not rows:
            print(
                f"🛑 {page}페이지에 데이터가 없습니다. 모든 페이지 수집 완료!"
            )
            break
    ...
    # 전체 데이터 ctr_items_db.json 저장
    with open("ctr_items_db.json", "w", encoding="utf-8") as f:
        json.dump(items, f, ensure_ascii=False, indent=4)

    print(
        f"\n🎉 총 {page - 1}개 페이지에서 총 {len(items)}개 아이템 수집 및"
        " 저장 완료!"
    )
